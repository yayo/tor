...
open(F, "/tmp/portage.tmp/portage/net-vpn/tor-0.3.1.5_alpha/work/tor-0.3.1.5-alpha/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/tmp/portage.tmp/portage/net-vpn/tor-0.3.1.5_alpha/work/tor-0.3.1.5-alpha/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/tmp/portage.tmp/portage/net-vpn/tor-0.3.1.5_alpha/work/tor-0.3.1.5-alpha/doc/tor.1.txt") or die;
while (<F>) {
